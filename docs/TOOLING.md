...
The template ships **no live CI** by design (to stay provider-agnostic), but includes a ready GitHub Actions starter at **`.github/workflows/ci.yml.example`**. Enable it by renaming to `ci.yml`. For *why* CI is structured the way it is — layered gates, gate vs advisory, where a new check belongs — see **[CI.md](CI.md)**. It mirrors the local gate:
...
CI also includes a **duplicate-image gate** (`ci.yml` → `duplicate-images`): on a PR it fails only on duplicates the PR *introduces*, on a push to `main` it fails on *any* duplicate. A weekly **`hygiene.yml`** runs advisory scans — unused images, unused Swift code ([Periphery](https://github.com/peripheryapp/periphery), tuned in `.periphery.yml`) — plus a tool self-test. Details: **[IMAGE_HYGIENE.md](IMAGE_HYGIENE.md)** and **[DEAD_CODE.md](DEAD_CODE.md)**.
