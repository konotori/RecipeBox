...
CI also includes a **duplicate-image gate** (`ci.yml` → `duplicate-images`): on a PR it fails only on duplicates the PR *introduces*, on a push to `main` it fails on *any* duplicate. A weekly **`hygiene.yml`** runs the unused-image scan + a tool self-test. What each image check does and does **not** cover is in **[IMAGE_HYGIENE.md](IMAGE_HYGIENE.md)**.
